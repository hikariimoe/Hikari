...
export class Logger extends BuiltinLogger {
    ...
    public constructor(client: Hikari) {
        super(LogLevel[client.configuration.bot.logging_level as keyof typeof LogLevel]);
    ...
    private preprocess(format: string, values: readonly unknown[]) {
        ...
        return format.replace("{date}", date).replace("{values}", values.join(" "));
    ...
    private getFormatString(level: keyof typeof LogLevel) {
        ...
        return `${chalk.hex(bubbleColor)("•")} ${chalk.bgHex(color).bold(` ${level.toUpperCase()} `)} ${chalk.hex("#484a49").italic("{date}")} • ${chalk.hex("#fafafa")("{values}")}`;
    }
}