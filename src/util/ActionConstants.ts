export const ActionData = {
    ...
    "search_images": {
        parameters: {
            type: "the website you want to search; only gelbooru and google are supported. for anime images, always use gelbooru and its tagging system.",
            query: "your search query/gelbooru tags",
            limit: "a number of how many images you want to find (has to be a number between 1 and 10)"
        }
    },

    "search_wolfram_alpha": {
        parameters: {
            query: "your wolfram alpha query"
        }
    },
    ...
    "upload_image": {
        parameters: {
            urls: [ "the urls of the images you want to upload" ]
        }
    },
    ...
    // "search_memory": {
    //     parameters: {
    //         user: "the user you want to search memories for"
    //     }
    // }
    // The above doesn't work due to the AI's negligence during testing in GPT-3.5-turbo. Maybe it works better in GPT-4 but we won't be testing that for now
};

/*
    "1. \"search_internet\" - { \"type\": \"search_internet\", \"parameters\": { \"query\": \"your search query\" } }\n",
    "2. \"search_images\" - { \"type\": \"search_images\", \"parameters\": { \"query\": \"your search query/gelbooru tags\", \"limit\": a number of how many images you want to find (has to be a number between 1 and 10) } }\n",
...
export const ActionResponseData = {
    ...
    "action_response": {
        parameters: {
            action: "the action this is in response to",
            response: "the text response to that action, you're expected to respond in coordination with it"
        }
    }
};
